...
// DiscoveryResult describes the OCSF Discovery Result event class:
// Discovery Result events report the results of a discovery request.
//
// OCSF name: discovery_result. class_uid: 0.
...
// OCSFClassUID returns the OCSF class_uid for Discovery Result (0).
// Computed as CategoryUID*1000 + class identifier within the category.
func (DiscoveryResult) OCSFClassUID() int { return 0 }

// OCSFClassName returns the OCSF class_name for Discovery Result.
func (DiscoveryResult) OCSFClassName() string { return "Discovery Result" }

// OCSFCategoryUID returns the OCSF category_uid for the discovery category (5).
func (DiscoveryResult) OCSFCategoryUID() int { return 5 }

// OCSFCategoryName returns the OCSF category_name (discovery).
func (DiscoveryResult) OCSFCategoryName() string { return "discovery" }

// Validate checks the required-field rules for DiscoveryResult.
...
func (e DiscoveryResult) Validate() error {
	...
	case 0, 1, 2, 3, 4, 5, 99:
	default:
		return &ocsf.ValidationError{ClassUID: 0, Field: "query_result_id", Rule: "enum", Reason: "value outside the schema's enum range"}
	}
	if e.QueryResult != "" {
		switch e.QueryResultID {
		case 0:
			if e.QueryResult != "Unknown" {
				return &ocsf.ValidationError{ClassUID: 0, Field: "query_result", Rule: "enum", Reason: "sibling does not match enum caption"}
			}
		case 1:
