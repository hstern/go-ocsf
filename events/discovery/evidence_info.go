...
// EvidenceInfo describes the OCSF Live Evidence Info event class: Data
// collected directly from devices that represents forensic information
// pulled, queried, or discovered from devices that may indicate
// malicious activity. It contains a number of child objects, each
// representing a distinct evidence domain (network connections, file
// artifacts, registry entries, etc.). When mapping raw telemetry data
// users should select Query Evidence and then the appropriate child
// object that best matches the evidence type.
//
// OCSF name: evidence_info. class_uid: 5040.
type EvidenceInfo struct {
	...
	// QueryEvidence is the Query Evidence. The specific resulting evidence
	// information that was queried or discovered based on the query type.
	// Contains various child objects corresponding to the query_type_id
	// values.
	//
	// OCSF: query_evidence (type query_evidence, requirement required)
...
// OCSFClassUID returns the OCSF class_uid for Live Evidence Info (5040).
// Computed as CategoryUID*1000 + class identifier within the category.
func (EvidenceInfo) OCSFClassUID() int { return 5040 }

// OCSFClassName returns the OCSF class_name for Live Evidence Info.
func (EvidenceInfo) OCSFClassName() string { return "Live Evidence Info" }

// OCSFCategoryUID returns the OCSF category_uid for the discovery category (5).
func (EvidenceInfo) OCSFCategoryUID() int { return 5 }

// OCSFCategoryName returns the OCSF category_name (discovery).
func (EvidenceInfo) OCSFCategoryName() string { return "discovery" }

// Validate checks the required-field rules for EvidenceInfo.
// Returns the first violation found, or nil if all required fields are present.
func (e EvidenceInfo) Validate() error {
	if e.Cloud == nil {
		return &ocsf.ValidationError{ClassUID: 5040, Field: "cloud", Rule: "required", Reason: "required field is missing"}
	}
	if e.Device == nil {
		return &ocsf.ValidationError{ClassUID: 5040, Field: "device", Rule: "required", Reason: "required field is missing"}
	}
	if e.Metadata == nil {
		return &ocsf.ValidationError{ClassUID: 5040, Field: "metadata", Rule: "required", Reason: "required field is missing"}
	}
	if len(e.Osint) == 0 {
		return &ocsf.ValidationError{ClassUID: 5040, Field: "osint", Rule: "required", Reason: "required field is missing"}
	}
	if e.QueryEvidence == nil {
		return &ocsf.ValidationError{ClassUID: 5040, Field: "query_evidence", Rule: "required", Reason: "required field is missing"}
	}
	switch e.ActionID {
	case 0, 1, 2, 3, 4, 99:
	default:
		return &ocsf.ValidationError{ClassUID: 5040, Field: "action_id", Rule: "enum", Reason: "value outside the schema's enum range"}
	}
	if e.Action != "" {
		switch e.ActionID {
		case 0:
			if e.Action != "Unknown" {
				return &ocsf.ValidationError{ClassUID: 5040, Field: "action", Rule: "enum", Reason: "sibling does not match enum caption"}
			}
		case 1:
			if e.Action != "Allowed" {
				return &ocsf.ValidationError{ClassUID: 5040, Field: "action", Rule: "enum", Reason: "sibling does not match enum caption"}
	...
	case 0, 1, 99:
	default:
		return &ocsf.ValidationError{ClassUID: 5040, Field: "activity_id", Rule: "enum", Reason: "value outside the schema's enum range"}
	}
	if e.ActivityName != "" {
		switch e.ActivityID {
		case 0:
			if e.ActivityName != "Unknown" {
				return &ocsf.ValidationError{ClassUID: 5040, Field: "activity_name", Rule: "enum", Reason: "sibling does not match enum caption"}
			}
		case 1:
	...
	case 0, 1, 2, 3, 99:
	default:
		return &ocsf.ValidationError{ClassUID: 5040, Field: "confidence_id", Rule: "enum", Reason: "value outside the schema's enum range"}
	}
	if e.Confidence != "" {
		switch e.ConfidenceID {
		case 0:
			if e.Confidence != "Unknown" {
				return &ocsf.ValidationError{ClassUID: 5040, Field: "confidence", Rule: "enum", Reason: "sibling does not match enum caption"}
			}
		case 1:
			if e.Confidence != "Low" {
				return &ocsf.ValidationError{ClassUID: 5040, Field: "confidence", Rule: "enum", Reason: "sibling does not match enum caption"}
	...
	default:
		return &ocsf.ValidationError{ClassUID: 5040, Field: "disposition_id", Rule: "enum", Reason: "value outside the schema's enum range"}
	}
	if e.Disposition != "" {
		switch e.DispositionID {
		case 0:
			if e.Disposition != "Unknown" {
				return &ocsf.ValidationError{ClassUID: 5040, Field: "disposition", Rule: "enum", Reason: "sibling does not match enum caption"}
			}
		case 1:
	...
	case 0, 1, 2, 3, 4, 5, 99:
	default:
		return &ocsf.ValidationError{ClassUID: 5040, Field: "query_result_id", Rule: "enum", Reason: "value outside the schema's enum range"}
	}
	if e.QueryResult != "" {
		switch e.QueryResultID {
		case 0:
			if e.QueryResult != "Unknown" {
				return &ocsf.ValidationError{ClassUID: 5040, Field: "query_result", Rule: "enum", Reason: "sibling does not match enum caption"}
			}
		case 1:
	...
	case 0, 1, 2, 3, 4, 99:
	default:
		return &ocsf.ValidationError{ClassUID: 5040, Field: "risk_level_id", Rule: "enum", Reason: "value outside the schema's enum range"}
	}
	if e.RiskLevel != "" {
		switch e.RiskLevelID {
		case 0:
			if e.RiskLevel != "Info" {
				return &ocsf.ValidationError{ClassUID: 5040, Field: "risk_level", Rule: "enum", Reason: "sibling does not match enum caption"}
			}
		case 1:
			if e.RiskLevel != "Low" {
				return &ocsf.ValidationError{ClassUID: 5040, Field: "risk_level", Rule: "enum", Reason: "sibling does not match enum caption"}
	...
	default:
		return &ocsf.ValidationError{ClassUID: 5040, Field: "severity_id", Rule: "enum", Reason: "value outside the schema's enum range"}
	}
	if e.Severity != "" {
		switch e.SeverityID {
		case 0:
			if e.Severity != "Unknown" {
				return &ocsf.ValidationError{ClassUID: 5040, Field: "severity", Rule: "enum", Reason: "sibling does not match enum caption"}
			}
		case 1:
	...
	case 0, 1, 2, 99:
	default:
		return &ocsf.ValidationError{ClassUID: 5040, Field: "status_id", Rule: "enum", Reason: "value outside the schema's enum range"}
	}
	if e.Status != "" {
		switch e.StatusID {
		case 0:
			if e.Status != "Unknown" {
				return &ocsf.ValidationError{ClassUID: 5040, Field: "status", Rule: "enum", Reason: "sibling does not match enum caption"}
			}
		case 1:
			if e.Status != "Success" {
				return &ocsf.ValidationError{ClassUID: 5040, Field: "status", Rule: "enum", Reason: "sibling does not match enum caption"}
...
func init() {
	ocsf.RegisterClass(5040, func() ocsf.Event { return &EvidenceInfo{} })
}
