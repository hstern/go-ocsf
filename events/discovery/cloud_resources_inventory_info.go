...
// CloudResourcesInventoryInfo describes the OCSF Cloud Resources
// Inventory Info event class: Cloud Resources Inventory Info events
// report cloud asset inventory data. This data can be either logged or
// proactively collected. For example, use this event class when creating
// an inventory of cloud resource information from a Configuration
// Management Database (CMDB), Cyber Asset Attack Surface Management
// (CAASM), direct public cloud service provider APIs,
// Software-as-a-Service (SaaS) APIs, or otherwise.
//
// OCSF name: cloud_resources_inventory_info. class_uid: 5023.
type CloudResourcesInventoryInfo struct {
	...
	// Cloud is the Cloud. Cloud service provider or SaaS platform metadata
	// about the cloud resource(s) that are being discovered by an inventory
	// process.
	//
	// OCSF: cloud (type cloud, requirement recommended)
	...
	// Container is the Container. A cloud-based container image or running
	// container discovered by an inventory process.
	//
	...
	// Database is the Database. A cloud-based database discovered by an
	// inventory process.
	//
	// OCSF: database (type database, requirement recommended)
	Database *objects.Database `json:"database,omitempty"`

	// Databucket is the Databucket. A cloud-based data bucket or other
	// object storage discovered by an inventory process.
	//
	...
	// Idp is the Identity Provider. The Identity Provider that is being
	// discovered by an inventory process, or that is related to the cloud
	// resource(s) being discovered by an inventory process.
	//
	// OCSF: idp (type idp, requirement recommended)
	...
	// Region is the Region. The cloud region where the resource is located,
	// e.g., us-isof-south-1, eastus2, us-central1, etc.
	//
	// OCSF: region (type string_t, requirement recommended)
	Region string `json:"region,omitempty"`

	// Resources is the Cloud Resources. The cloud resource(s) that are being
	// discovered by an inventory process. Use this object if there is not a
	// direct object match in the class.
	//
	...
	// Table is the Table. A cloud-based database table discovered by an
	// inventory process.
	//
...
// OCSFClassUID returns the OCSF class_uid for Cloud Resources Inventory Info (5023).
// Computed as CategoryUID*1000 + class identifier within the category.
func (CloudResourcesInventoryInfo) OCSFClassUID() int { return 5023 }

// OCSFClassName returns the OCSF class_name for Cloud Resources Inventory Info.
func (CloudResourcesInventoryInfo) OCSFClassName() string { return "Cloud Resources Inventory Info" }

// OCSFCategoryUID returns the OCSF category_uid for the discovery category (5).
func (CloudResourcesInventoryInfo) OCSFCategoryUID() int { return 5 }

// OCSFCategoryName returns the OCSF category_name (discovery).
func (CloudResourcesInventoryInfo) OCSFCategoryName() string { return "discovery" }

// Validate checks the required-field rules for CloudResourcesInventoryInfo.
...
func (e CloudResourcesInventoryInfo) Validate() error {
	if e.Metadata == nil {
		return &ocsf.ValidationError{ClassUID: 5023, Field: "metadata", Rule: "required", Reason: "required field is missing"}
	}
	if len(e.Osint) == 0 {
		return &ocsf.ValidationError{ClassUID: 5023, Field: "osint", Rule: "required", Reason: "required field is missing"}
	}
	switch e.ActionID {
	case 0, 1, 2, 3, 4, 99:
	default:
		return &ocsf.ValidationError{ClassUID: 5023, Field: "action_id", Rule: "enum", Reason: "value outside the schema's enum range"}
	}
	if e.Action != "" {
		switch e.ActionID {
		case 0:
			if e.Action != "Unknown" {
				return &ocsf.ValidationError{ClassUID: 5023, Field: "action", Rule: "enum", Reason: "sibling does not match enum caption"}
			}
		case 1:
			if e.Action != "Allowed" {
				return &ocsf.ValidationError{ClassUID: 5023, Field: "action", Rule: "enum", Reason: "sibling does not match enum caption"}
	...
	case 0, 1, 2, 3, 99:
	default:
		return &ocsf.ValidationError{ClassUID: 5023, Field: "activity_id", Rule: "enum", Reason: "value outside the schema's enum range"}
	}
	if e.ActivityName != "" {
		switch e.ActivityID {
		case 0:
			if e.ActivityName != "Unknown" {
				return &ocsf.ValidationError{ClassUID: 5023, Field: "activity_name", Rule: "enum", Reason: "sibling does not match enum caption"}
			}
		case 1:
	...
	case 0, 1, 2, 3, 99:
	default:
		return &ocsf.ValidationError{ClassUID: 5023, Field: "confidence_id", Rule: "enum", Reason: "value outside the schema's enum range"}
	}
	if e.Confidence != "" {
		switch e.ConfidenceID {
		case 0:
			if e.Confidence != "Unknown" {
				return &ocsf.ValidationError{ClassUID: 5023, Field: "confidence", Rule: "enum", Reason: "sibling does not match enum caption"}
			}
		case 1:
			if e.Confidence != "Low" {
				return &ocsf.ValidationError{ClassUID: 5023, Field: "confidence", Rule: "enum", Reason: "sibling does not match enum caption"}
	...
	default:
		return &ocsf.ValidationError{ClassUID: 5023, Field: "disposition_id", Rule: "enum", Reason: "value outside the schema's enum range"}
	}
	if e.Disposition != "" {
		switch e.DispositionID {
		case 0:
			if e.Disposition != "Unknown" {
				return &ocsf.ValidationError{ClassUID: 5023, Field: "disposition", Rule: "enum", Reason: "sibling does not match enum caption"}
			}
		case 1:
	...
	case 0, 1, 2, 3, 4, 99:
	default:
		return &ocsf.ValidationError{ClassUID: 5023, Field: "risk_level_id", Rule: "enum", Reason: "value outside the schema's enum range"}
	}
	if e.RiskLevel != "" {
		switch e.RiskLevelID {
		case 0:
			if e.RiskLevel != "Info" {
				return &ocsf.ValidationError{ClassUID: 5023, Field: "risk_level", Rule: "enum", Reason: "sibling does not match enum caption"}
			}
		case 1:
			if e.RiskLevel != "Low" {
				return &ocsf.ValidationError{ClassUID: 5023, Field: "risk_level", Rule: "enum", Reason: "sibling does not match enum caption"}
	...
	default:
		return &ocsf.ValidationError{ClassUID: 5023, Field: "severity_id", Rule: "enum", Reason: "value outside the schema's enum range"}
	}
	if e.Severity != "" {
		switch e.SeverityID {
		case 0:
			if e.Severity != "Unknown" {
				return &ocsf.ValidationError{ClassUID: 5023, Field: "severity", Rule: "enum", Reason: "sibling does not match enum caption"}
			}
		case 1:
	...
	case 0, 1, 2, 3, 4, 5, 6, 99:
	default:
		return &ocsf.ValidationError{ClassUID: 5023, Field: "status_id", Rule: "enum", Reason: "value outside the schema's enum range"}
	}
	if e.Status != "" {
		switch e.StatusID {
		case 0:
			if e.Status != "Unknown" {
				return &ocsf.ValidationError{ClassUID: 5023, Field: "status", Rule: "enum", Reason: "sibling does not match enum caption"}
			}
		case 1:
			if e.Status != "New" {
				return &ocsf.ValidationError{ClassUID: 5023, Field: "status", Rule: "enum", Reason: "sibling does not match enum caption"}
	...
	if !(e.Cloud != nil || e.Container != nil || e.Database != nil || e.Databucket != nil || e.Idp != nil || len(e.Resources) != 0 || e.Table != nil) {
		return &ocsf.ValidationError{ClassUID: 5023, Field: "cloud,container,database,databucket,idp,resources,table", Rule: "constraint", Reason: "at_least_one: at least one of these fields must be set"}
	}
	return nil
}

func init() {
	ocsf.RegisterClass(5023, func() ocsf.Event { return &CloudResourcesInventoryInfo{} })
}
