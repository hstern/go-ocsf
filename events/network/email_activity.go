...
type EmailActivity struct {
	...
	// Attempt is the Attempt. The attempt number for attempting to deliver
	// the email.
	//
	// OCSF: attempt (type integer_t, requirement optional)
	...
	// Direction is the Direction. The direction of the email, as defined by
	// the direction_id value.
	//
	// OCSF: direction (type string_t, requirement optional)
	Direction string `json:"direction,omitempty"`

	// DirectionID is the Direction ID. The direction of the email relative
	// to the scanning host or organization.Email scanned at an internet
	// gateway might be characterized as inbound to the organization from the
	// Internet, outbound from the organization to the Internet, or internal
	// within the organization. Email scanned at a workstation might be
	...
	// DstEndpoint is the Destination Endpoint. The responder (server)
	// receiving the email.
	//
	...
	//
	// OCSF: email (type email, requirement required)
	Email *objects.Email `json:"email"`

	// EmailAuth is the Email Authentication. The SPF, DKIM and DMARC
	// attributes of an email.
	//
	// OCSF: email_auth (type email_auth, requirement recommended)
	...
	// SMTPHello is the SMTP Hello. The value of the SMTP HELO or EHLO
...
// OCSFClassUID returns the OCSF class_uid for Email Activity (4009).
// Computed as CategoryUID*1000 + class identifier within the category.
func (EmailActivity) OCSFClassUID() int { return 4009 }

// OCSFClassName returns the OCSF class_name for Email Activity.
func (EmailActivity) OCSFClassName() string { return "Email Activity" }

// OCSFCategoryUID returns the OCSF category_uid for the network category (4).
func (EmailActivity) OCSFCategoryUID() int { return 4 }

// OCSFCategoryName returns the OCSF category_name (network).
func (EmailActivity) OCSFCategoryName() string { return "network" }

// Validate checks the required-field rules for EmailActivity.
// Returns the first violation found, or nil if all required fields are present.
func (e EmailActivity) Validate() error {
	if e.Cloud == nil {
		return &ocsf.ValidationError{ClassUID: 4009, Field: "cloud", Rule: "required", Reason: "required field is missing"}
	}
	if e.Email == nil {
		return &ocsf.ValidationError{ClassUID: 4009, Field: "email", Rule: "required", Reason: "required field is missing"}
	}
	if e.Metadata == nil {
		return &ocsf.ValidationError{ClassUID: 4009, Field: "metadata", Rule: "required", Reason: "required field is missing"}
	}
	if len(e.Osint) == 0 {
	...
	case 0:
	default:
		return &ocsf.ValidationError{ClassUID: 4009, Field: "category_uid", Rule: "enum", Reason: "value outside the schema's enum range"}
	}
	if e.CategoryName != "" {
		switch e.CategoryUID {
		case 0:
			if e.CategoryName != "Uncategorized" {
				return &ocsf.ValidationError{ClassUID: 4009, Field: "category_name", Rule: "enum", Reason: "sibling does not match enum caption"}
			}
		}
	}
	switch e.ClassUID {
	case 0:
	default:
		return &ocsf.ValidationError{ClassUID: 4009, Field: "class_uid", Rule: "enum", Reason: "value outside the schema's enum range"}
	}
	if e.ClassName != "" {
		switch e.ClassUID {
		case 0:
			if e.ClassName != "Base Event" {
				return &ocsf.ValidationError{ClassUID: 4009, Field: "class_name", Rule: "enum", Reason: "sibling does not match enum caption"}
			}
		}
	}
	...
	default:
		return &ocsf.ValidationError{ClassUID: 4009, Field: "disposition_id", Rule: "enum", Reason: "value outside the schema's enum range"}
	}
	if e.Disposition != "" {
		switch e.DispositionID {
		case 0:
			if e.Disposition != "Unknown" {
				return &ocsf.ValidationError{ClassUID: 4009, Field: "disposition", Rule: "enum", Reason: "sibling does not match enum caption"}
			}
		case 1:
	...
	default:
		return &ocsf.ValidationError{ClassUID: 4009, Field: "severity_id", Rule: "enum", Reason: "value outside the schema's enum range"}
	}
	if e.Severity != "" {
		switch e.SeverityID {
		case 0:
			if e.Severity != "Unknown" {
				return &ocsf.ValidationError{ClassUID: 4009, Field: "severity", Rule: "enum", Reason: "sibling does not match enum caption"}
			}
		case 1:
...
func init() {
	ocsf.RegisterClass(4009, func() ocsf.Event { return &EmailActivity{} })
}
