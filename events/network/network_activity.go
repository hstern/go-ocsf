...
// NetworkActivity describes the OCSF Network Activity event class:
// Network Activity events report network connection and traffic
// activity.
//
// OCSF name: network_activity. class_uid: 4001.
type NetworkActivity struct {
	...
	// URL is the URL. The URL details relevant to the network traffic.
	//
	// OCSF: url (type url, requirement recommended)
	URL *objects.URL `json:"url,omitempty"`
}

// OCSFClassUID returns the OCSF class_uid for Network Activity (4001).
// Computed as CategoryUID*1000 + class identifier within the category.
func (NetworkActivity) OCSFClassUID() int { return 4001 }

// OCSFClassName returns the OCSF class_name for Network Activity.
func (NetworkActivity) OCSFClassName() string { return "Network Activity" }

// OCSFCategoryUID returns the OCSF category_uid for the network category (4).
func (NetworkActivity) OCSFCategoryUID() int { return 4 }
...
func (e NetworkActivity) Validate() error {
	...
	default:
		return &ocsf.ValidationError{ClassUID: 4001, Field: "activity_id", Rule: "enum", Reason: "value outside the schema's enum range"}
	}
	if e.ActivityName != "" {
		switch e.ActivityID {
		case 0:
	...
	case 0:
	default:
		return &ocsf.ValidationError{ClassUID: 4001, Field: "category_uid", Rule: "enum", Reason: "value outside the schema's enum range"}
	}
	if e.CategoryName != "" {
		switch e.CategoryUID {
		case 0:
			if e.CategoryName != "Uncategorized" {
				return &ocsf.ValidationError{ClassUID: 4001, Field: "category_name", Rule: "enum", Reason: "sibling does not match enum caption"}
			}
		}
	}
	switch e.ClassUID {
	case 0:
	default:
		return &ocsf.ValidationError{ClassUID: 4001, Field: "class_uid", Rule: "enum", Reason: "value outside the schema's enum range"}
	}
	if e.ClassName != "" {
		switch e.ClassUID {
		case 0:
			if e.ClassName != "Base Event" {
				return &ocsf.ValidationError{ClassUID: 4001, Field: "class_name", Rule: "enum", Reason: "sibling does not match enum caption"}
			}
		}
	}
	...
	default:
		return &ocsf.ValidationError{ClassUID: 4001, Field: "disposition_id", Rule: "enum", Reason: "value outside the schema's enum range"}
	}
	if e.Disposition != "" {
		switch e.DispositionID {
		case 0:
			if e.Disposition != "Unknown" {
				return &ocsf.ValidationError{ClassUID: 4001, Field: "disposition", Rule: "enum", Reason: "sibling does not match enum caption"}
			}
		case 1:
	...
	default:
		return &ocsf.ValidationError{ClassUID: 4001, Field: "severity_id", Rule: "enum", Reason: "value outside the schema's enum range"}
	}
	if e.Severity != "" {
		switch e.SeverityID {
		case 0:
			if e.Severity != "Unknown" {
				return &ocsf.ValidationError{ClassUID: 4001, Field: "severity", Rule: "enum", Reason: "sibling does not match enum caption"}
			}
		case 1:
