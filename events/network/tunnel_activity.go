...
// TunnelActivity describes the OCSF Tunnel Activity event class: Tunnel
// Activity events report secure tunnel establishment (such as VPN),
// teardowns, renewals, and other network tunnel specific actions.
//
// OCSF name: tunnel_activity. class_uid: 4014.
type TunnelActivity struct {
	...
	// ProtocolName is the Tunnel Protocol. The networking protocol
	// associated with the tunnel. E.g. IPSec, SSL, GRE.
	//
	...
	// Traffic is the Traffic. Traffic refers to the amount of data moving
	// across the tunnel at a given point of time. Ex: bytes_in and
	// bytes_out.
	//
	// OCSF: traffic (type network_traffic, requirement optional)
	Traffic *objects.NetworkTraffic `json:"traffic,omitempty"`

	// TunnelInterface is the Tunnel Interface. The information about the
	// virtual tunnel interface, e.g. utun0. This is usually associated with
	// the private (rfc-1918) ip of the tunnel.
	//
	// OCSF: tunnel_interface (type network_interface, requirement recommended)
	TunnelInterface *objects.NetworkInterface `json:"tunnel_interface,omitempty"`

	// TunnelType is the Type. The tunnel type. Example: Split or Full.
	//
	// OCSF: tunnel_type (type string_t, requirement recommended)
	...
	// User is the User. The user associated with the tunnel activity.
	//
	// OCSF: user (type user, requirement recommended)
	User *objects.User `json:"user,omitempty"`
}

// OCSFClassUID returns the OCSF class_uid for Tunnel Activity (4014).
// Computed as CategoryUID*1000 + class identifier within the category.
func (TunnelActivity) OCSFClassUID() int { return 4014 }

// OCSFClassName returns the OCSF class_name for Tunnel Activity.
func (TunnelActivity) OCSFClassName() string { return "Tunnel Activity" }

// OCSFCategoryUID returns the OCSF category_uid for the network category (4).
func (TunnelActivity) OCSFCategoryUID() int { return 4 }

// OCSFCategoryName returns the OCSF category_name (network).
func (TunnelActivity) OCSFCategoryName() string { return "network" }

// Validate checks the required-field rules for TunnelActivity.
// Returns the first violation found, or nil if all required fields are present.
func (e TunnelActivity) Validate() error {
	if e.Cloud == nil {
		return &ocsf.ValidationError{ClassUID: 4014, Field: "cloud", Rule: "required", Reason: "required field is missing"}
	}
	if e.Metadata == nil {
		return &ocsf.ValidationError{ClassUID: 4014, Field: "metadata", Rule: "required", Reason: "required field is missing"}
	}
	if len(e.Osint) == 0 {
	...
	default:
		return &ocsf.ValidationError{ClassUID: 4014, Field: "activity_id", Rule: "enum", Reason: "value outside the schema's enum range"}
	}
	if e.ActivityName != "" {
		switch e.ActivityID {
		case 0:
			if e.ActivityName != "Unknown" {
				return &ocsf.ValidationError{ClassUID: 4014, Field: "activity_name", Rule: "enum", Reason: "sibling does not match enum caption"}
			}
		case 1:
	...
	case 0:
	default:
		return &ocsf.ValidationError{ClassUID: 4014, Field: "category_uid", Rule: "enum", Reason: "value outside the schema's enum range"}
	}
	if e.CategoryName != "" {
		switch e.CategoryUID {
		case 0:
			if e.CategoryName != "Uncategorized" {
				return &ocsf.ValidationError{ClassUID: 4014, Field: "category_name", Rule: "enum", Reason: "sibling does not match enum caption"}
			}
		}
	}
	switch e.ClassUID {
	case 0:
	default:
		return &ocsf.ValidationError{ClassUID: 4014, Field: "class_uid", Rule: "enum", Reason: "value outside the schema's enum range"}
	}
	if e.ClassName != "" {
		switch e.ClassUID {
		case 0:
			if e.ClassName != "Base Event" {
				return &ocsf.ValidationError{ClassUID: 4014, Field: "class_name", Rule: "enum", Reason: "sibling does not match enum caption"}
			}
		}
	}
	...
	default:
		return &ocsf.ValidationError{ClassUID: 4014, Field: "severity_id", Rule: "enum", Reason: "value outside the schema's enum range"}
	}
	if e.Severity != "" {
		switch e.SeverityID {
		case 0:
			if e.Severity != "Unknown" {
				return &ocsf.ValidationError{ClassUID: 4014, Field: "severity", Rule: "enum", Reason: "sibling does not match enum caption"}
			}
		case 1:
	...
	if e.TunnelType != "" {
		...
		case 1:
			if e.TunnelType != "Split Tunnel" {
				return &ocsf.ValidationError{ClassUID: 4014, Field: "tunnel_type", Rule: "enum", Reason: "sibling does not match enum caption"}
			}
		case 2:
			if e.TunnelType != "Full Tunnel" {
				return &ocsf.ValidationError{ClassUID: 4014, Field: "tunnel_type", Rule: "enum", Reason: "sibling does not match enum caption"}
			}
		}
	}
