...
// Network describes the OCSF Network event class: Network event is a
// generic event that defines a set of attributes available in the
// Network category.
//
// OCSF name: network. class_uid: 0.
type Network struct {
...
// OCSFClassUID returns the OCSF class_uid for Network (0).
// Computed as CategoryUID*1000 + class identifier within the category.
func (Network) OCSFClassUID() int { return 0 }

// OCSFClassName returns the OCSF class_name for Network.
func (Network) OCSFClassName() string { return "Network" }

// OCSFCategoryUID returns the OCSF category_uid for the network category (4).
func (Network) OCSFCategoryUID() int { return 4 }

// OCSFCategoryName returns the OCSF category_name (network).
func (Network) OCSFCategoryName() string { return "network" }

// Validate checks the required-field rules for Network.
// Returns the first violation found, or nil if all required fields are present.
func (e Network) Validate() error {
	if e.Cloud == nil {
	...
	if e.ObservationPointID != nil {
		...
		case 0, 1, 2, 3, 4, 99:
		default:
			return &ocsf.ValidationError{ClassUID: 0, Field: "observation_point_id", Rule: "enum", Reason: "value outside the schema's enum range"}
		}
	}
	if e.ObservationPoint != "" && e.ObservationPointID != nil {
		...
		case 0:
			if e.ObservationPoint != "Unknown" {
				return &ocsf.ValidationError{ClassUID: 0, Field: "observation_point", Rule: "enum", Reason: "sibling does not match enum caption"}
			}
		case 1:
	...
	if !(e.DstEndpoint != nil || e.SrcEndpoint != nil) {
		return &ocsf.ValidationError{ClassUID: 0, Field: "dst_endpoint,src_endpoint", Rule: "constraint", Reason: "at_least_one: at least one of these fields must be set"}
	}
	return nil
}
