...
// HTTPActivity describes the OCSF HTTP Activity event class: HTTP
// Activity events report HTTP connection and traffic information.
//
// OCSF name: http_activity. class_uid: 4002.
type HTTPActivity struct {
	...
	// File is the File. The file that is the target of the HTTP activity.
	//
	...
	// HTTPCookies is the HTTP Cookies. The cookies object describes details
	// about HTTP cookies
	//
	// OCSF: http_cookies (type []http_cookie, requirement recommended)
	HTTPCookies []objects.HTTPCookie `json:"http_cookies,omitempty"`

	// HTTPRequest is the HTTP Request. The HTTP Request Object documents
	// attributes of a request made to a web server.
	//
	// OCSF: http_request (type http_request, requirement recommended)
	HTTPRequest *objects.HTTPRequest `json:"http_request,omitempty"`

	// HTTPResponse is the HTTP Response. The HTTP Response from a web server
	// to a requester.
	//
	...
	// HTTPStatus is the HTTP Status. The Hypertext Transfer Protocol (HTTP)
	// status code returned to the client.
	//
	// OCSF: http_status (type integer_t, requirement recommended)
	...
	// Trace is the Trace. The trace object contains information about
	// distributed traces which are critical to observability and describe
	// how requests move through a system, capturing each step's timing and
	// status.
	//
	// OCSF: trace (type trace, requirement recommended)
...
// OCSFClassUID returns the OCSF class_uid for HTTP Activity (4002).
// Computed as CategoryUID*1000 + class identifier within the category.
func (HTTPActivity) OCSFClassUID() int { return 4002 }

// OCSFClassName returns the OCSF class_name for HTTP Activity.
func (HTTPActivity) OCSFClassName() string { return "HTTP Activity" }

// OCSFCategoryUID returns the OCSF category_uid for the network category (4).
func (HTTPActivity) OCSFCategoryUID() int { return 4 }

// OCSFCategoryName returns the OCSF category_name (network).
func (HTTPActivity) OCSFCategoryName() string { return "network" }

// Validate checks the required-field rules for HTTPActivity.
// Returns the first violation found, or nil if all required fields are present.
func (e HTTPActivity) Validate() error {
	if e.Cloud == nil {
		return &ocsf.ValidationError{ClassUID: 4002, Field: "cloud", Rule: "required", Reason: "required field is missing"}
	}
	if e.Metadata == nil {
		return &ocsf.ValidationError{ClassUID: 4002, Field: "metadata", Rule: "required", Reason: "required field is missing"}
	}
	if len(e.Osint) == 0 {
	...
	case 0, 1, 2, 3, 99:
	default:
		return &ocsf.ValidationError{ClassUID: 4002, Field: "confidence_id", Rule: "enum", Reason: "value outside the schema's enum range"}
	}
	if e.Confidence != "" {
		switch e.ConfidenceID {
		case 0:
			if e.Confidence != "Unknown" {
				return &ocsf.ValidationError{ClassUID: 4002, Field: "confidence", Rule: "enum", Reason: "sibling does not match enum caption"}
			}
		case 1:
			if e.Confidence != "Low" {
				return &ocsf.ValidationError{ClassUID: 4002, Field: "confidence", Rule: "enum", Reason: "sibling does not match enum caption"}
	...
	case 0, 1, 2, 3, 4, 99:
	default:
		return &ocsf.ValidationError{ClassUID: 4002, Field: "observation_point_id", Rule: "enum", Reason: "value outside the schema's enum range"}
	}
	if e.ObservationPoint != "" {
		switch e.ObservationPointID {
		case 0:
			if e.ObservationPoint != "Unknown" {
				return &ocsf.ValidationError{ClassUID: 4002, Field: "observation_point", Rule: "enum", Reason: "sibling does not match enum caption"}
			}
		case 1:
	...
	case 0, 1, 2, 3, 4, 99:
	default:
		return &ocsf.ValidationError{ClassUID: 4002, Field: "risk_level_id", Rule: "enum", Reason: "value outside the schema's enum range"}
	}
	if e.RiskLevel != "" {
		switch e.RiskLevelID {
		case 0:
			if e.RiskLevel != "Info" {
				return &ocsf.ValidationError{ClassUID: 4002, Field: "risk_level", Rule: "enum", Reason: "sibling does not match enum caption"}
			}
		case 1:
			if e.RiskLevel != "Low" {
				return &ocsf.ValidationError{ClassUID: 4002, Field: "risk_level", Rule: "enum", Reason: "sibling does not match enum caption"}
	...
	if !(e.HTTPRequest != nil || e.HTTPResponse != nil) {
		return &ocsf.ValidationError{ClassUID: 4002, Field: "http_request,http_response", Rule: "constraint", Reason: "at_least_one: at least one of these fields must be set"}
	}
	return nil
}

func init() {
	ocsf.RegisterClass(4002, func() ocsf.Event { return &HTTPActivity{} })
}
