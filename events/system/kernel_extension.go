...
// KernelExtension describes the OCSF Kernel Extension Activity event
// class: Kernel Extension events report when a driver/extension is
// loaded or unloaded into the kernel
//
// OCSF name: kernel_extension. class_uid: 1002.
...
func (KernelExtension) OCSFClassUID() int { return 1002 }

// OCSFClassName returns the OCSF class_name for Kernel Extension Activity.
func (KernelExtension) OCSFClassName() string { return "Kernel Extension Activity" }

// OCSFCategoryUID returns the OCSF category_uid for the system category (1).
func (KernelExtension) OCSFCategoryUID() int { return 1 }

// OCSFCategoryName returns the OCSF category_name (system).
func (KernelExtension) OCSFCategoryName() string { return "system" }

// Validate checks the required-field rules for KernelExtension.
// Returns the first violation found, or nil if all required fields are present.
func (e KernelExtension) Validate() error {
	if e.Actor == nil {
	...
	case 0:
	default:
		return &ocsf.ValidationError{ClassUID: 1002, Field: "category_uid", Rule: "enum", Reason: "value outside the schema's enum range"}
	}
	if e.CategoryName != "" {
		switch e.CategoryUID {
		case 0:
			if e.CategoryName != "Uncategorized" {
				return &ocsf.ValidationError{ClassUID: 1002, Field: "category_name", Rule: "enum", Reason: "sibling does not match enum caption"}
			}
		}
	}
	switch e.ClassUID {
	case 0:
	default:
		return &ocsf.ValidationError{ClassUID: 1002, Field: "class_uid", Rule: "enum", Reason: "value outside the schema's enum range"}
	}
	if e.ClassName != "" {
		switch e.ClassUID {
		case 0:
			if e.ClassName != "Base Event" {
				return &ocsf.ValidationError{ClassUID: 1002, Field: "class_name", Rule: "enum", Reason: "sibling does not match enum caption"}
			}
		}
	}
...
func init() {
	ocsf.RegisterClass(1002, func() ocsf.Event { return &KernelExtension{} })
}
