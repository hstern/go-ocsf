...
// ScheduledJobActivity describes the OCSF Scheduled Job Activity event
// class: Scheduled Job Activity events report activities related to
// scheduled jobs or tasks.
//
// OCSF name: scheduled_job_activity. class_uid: 1006.
type ScheduledJobActivity struct {
	...
	// Actor is the Actor. The actor that performed the activity on the job
	// object.
	//
...
// OCSFClassUID returns the OCSF class_uid for Scheduled Job Activity (1006).
// Computed as CategoryUID*1000 + class identifier within the category.
func (ScheduledJobActivity) OCSFClassUID() int { return 1006 }

// OCSFClassName returns the OCSF class_name for Scheduled Job Activity.
func (ScheduledJobActivity) OCSFClassName() string { return "Scheduled Job Activity" }

// OCSFCategoryUID returns the OCSF category_uid for the system category (1).
func (ScheduledJobActivity) OCSFCategoryUID() int { return 1 }
...
func (e ScheduledJobActivity) Validate() error {
	...
	default:
		return &ocsf.ValidationError{ClassUID: 1006, Field: "activity_id", Rule: "enum", Reason: "value outside the schema's enum range"}
	}
	if e.ActivityName != "" {
		switch e.ActivityID {
		case 0:
	...
	case 0:
	default:
		return &ocsf.ValidationError{ClassUID: 1006, Field: "category_uid", Rule: "enum", Reason: "value outside the schema's enum range"}
	}
	if e.CategoryName != "" {
		switch e.CategoryUID {
		case 0:
			if e.CategoryName != "Uncategorized" {
				return &ocsf.ValidationError{ClassUID: 1006, Field: "category_name", Rule: "enum", Reason: "sibling does not match enum caption"}
			}
		}
	}
	switch e.ClassUID {
	case 0:
	default:
		return &ocsf.ValidationError{ClassUID: 1006, Field: "class_uid", Rule: "enum", Reason: "value outside the schema's enum range"}
	}
	if e.ClassName != "" {
		switch e.ClassUID {
		case 0:
			if e.ClassName != "Base Event" {
				return &ocsf.ValidationError{ClassUID: 1006, Field: "class_name", Rule: "enum", Reason: "sibling does not match enum caption"}
			}
		}
	}
	...
	default:
		return &ocsf.ValidationError{ClassUID: 1006, Field: "disposition_id", Rule: "enum", Reason: "value outside the schema's enum range"}
	}
	if e.Disposition != "" {
		switch e.DispositionID {
		case 0:
			if e.Disposition != "Unknown" {
				return &ocsf.ValidationError{ClassUID: 1006, Field: "disposition", Rule: "enum", Reason: "sibling does not match enum caption"}
			}
		case 1:
	...
	default:
		return &ocsf.ValidationError{ClassUID: 1006, Field: "severity_id", Rule: "enum", Reason: "value outside the schema's enum range"}
	}
	if e.Severity != "" {
		switch e.SeverityID {
		case 0:
			if e.Severity != "Unknown" {
				return &ocsf.ValidationError{ClassUID: 1006, Field: "severity", Rule: "enum", Reason: "sibling does not match enum caption"}
			}
		case 1:
