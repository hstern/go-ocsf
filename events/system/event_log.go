...
// EventLog describes the OCSF Event Log Activity event class: Event Log
// Activity events report actions pertaining to the system's event
// logging service(s), such as disabling logging or clearing the log
// data.
//
// OCSF name: event_log. class_uid: 1008.
...
func (EventLog) OCSFClassUID() int { return 1008 }

// OCSFClassName returns the OCSF class_name for Event Log Activity.
func (EventLog) OCSFClassName() string { return "Event Log Activity" }

// OCSFCategoryUID returns the OCSF category_uid for the system category (1).
func (EventLog) OCSFCategoryUID() int { return 1 }
...
func (e EventLog) Validate() error {
	...
	case 0:
	default:
		return &ocsf.ValidationError{ClassUID: 1008, Field: "category_uid", Rule: "enum", Reason: "value outside the schema's enum range"}
	}
	if e.CategoryName != "" {
		switch e.CategoryUID {
		case 0:
			if e.CategoryName != "Uncategorized" {
				return &ocsf.ValidationError{ClassUID: 1008, Field: "category_name", Rule: "enum", Reason: "sibling does not match enum caption"}
			}
		}
	}
	switch e.ClassUID {
	case 0:
	default:
		return &ocsf.ValidationError{ClassUID: 1008, Field: "class_uid", Rule: "enum", Reason: "value outside the schema's enum range"}
	}
	if e.ClassName != "" {
		switch e.ClassUID {
		case 0:
			if e.ClassName != "Base Event" {
				return &ocsf.ValidationError{ClassUID: 1008, Field: "class_name", Rule: "enum", Reason: "sibling does not match enum caption"}
			}
		}
	}
