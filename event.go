...
type Event interface {
	...
	// Validate checks the event against the OCSF schema's
	// required-field, enum-membership, sibling-correspondence,
	// and class-level constraint rules. Returns the first
	// violation found as a *[ValidationError], or nil if the
	// event satisfies every rule.
	//
	// For known event classes (the codegen-emitted ones under
	// events/<category>/) Validate is generated by the codegen
	// pipeline and runs the schema's declared rules. For
	// [BaseEvent] — the forward-compat fallback for unknown
	// class_uid — Validate returns nil unconditionally, since
	// the library doesn't know the schema rules for an
	// unrecognized class.
	Validate() error
}

// Validate is a small package-level shim that calls
// e.Validate(). Useful when the consumer holds an Event-typed
// value and prefers the function-call form for symmetry with
// [Parse] (Parse, then Validate, then act).
//
// A nil Event returns a *ValidationError describing the
// nil-input case, not a Go nil-pointer panic — defensive
// because the natural call shape `Validate(Parse(...))` would
// otherwise crash if Parse returned (nil, error) and the
// caller forgot to check err.
func Validate(e Event) error {
	if e == nil {
		return &ValidationError{Reason: "event is nil"}
	}
	return e.Validate()
}
...
// Validate returns nil for BaseEvent. The fallback type
// represents events whose class_uid the library doesn't have
// a generated type for, so the schema's per-class required /
// enum / constraint rules are unknown to the library. Strict
// callers that want "unknown class_uid is an error" should
// type-assert the result of [Parse] to detect *BaseEvent
// before validating; see Parse's doc comment for the
// recommended pattern.
func (*BaseEvent) Validate() error { return nil }
